...
// FIX: A stable reference for the fallback array to satisfy useMemo dependencies
...
export const getServerSideProps = withPageAuthRequired({
  async getServerSideProps({ query }) {
    ...
    if (!BASE || !KEY) {
      return {
        // FIX: Cast the inner props object, not the wrapper!
...
export default function DashboardPage(props: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  // FIX: Using EMPTY_ROWS keeps the dependency stable
  const rows = "rows"