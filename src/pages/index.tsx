...
export const getServerSideProps = withPageAuthRequired({
  async getServerSideProps({ query }) {
    ...
    qs.set("experience", activeTab); // Pass the tab to the backend to filter!
...
export default function DashboardPage(props: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  const { kiosks, rows, activeTab, date_from, date_to } = props as OkProps;
            ...
            <tbody className="divide-y divide-gray-200">
              {rows.map((r) => (
                <tr key={r.kiosk_id} className="hover:bg-gray-50">
                  ...
                  {activeTab === "wallet" && (
                    <>
                      <td className="p-4 text-right border-l border-gray-100 tabular-nums">{r.avg_map_time_sec?.toFixed(1) || "—"}</td>
                      <td className="p-4 text-right tabular-nums">{r.avg_poi_popups_completed?.toFixed(1) || "—"}</td>
                      <td className="p-4 text-right tabular-nums text-gray-500">{r.avg_poi_popups_abandoned?.toFixed(1) || "—"}</td>
                      <td className="p-4 text-right tabular-nums">{r.avg_easter_eggs?.toFixed(1) || "—"}</td>
                      <td className="p-4 text-right tabular-nums">{r.back_to_map_sessions || 0}</td>
